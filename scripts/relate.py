...
gdelt_df = pd.read_csv("gdelt_5_years_filtered_updated.csv")

# Convert the '_id' column in wti_df and 'Date' column in gdelt_df to datetime
wti_df['_id'] = pd.to_datetime(wti_df['_id'])
gdelt_df['Updated_Date'] = pd.to_datetime(gdelt_df['Updated_Date'])
...
relation_df = gdelt_df[['Headline', 'Updated_Date']].drop_duplicates()

print(relation_df.shape)

# Convert 'Date' to string format for better readability
relation_df['Updated_Date'] = relation_df['Updated_Date'].dt.strftime('%Y-%m-%d')
