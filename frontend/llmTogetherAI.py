...
"""
    response = call_together_ai(prompt, key=TOGETHER_API_KEY_2, max_tokens=4096, temperature=0.7)
    if not response:
...
def process_user_query(user_question):
    ...
    print("\n🔍 Extracted Keywords:", keywords, file=sys.stderr)
...
if __name__ == "__main__":
    # --- API mode: if a question is passed as first arg, do one JSON response and exit ---
    if len(sys.argv) > 1:
        question = sys.argv[1]
        summary, headlines = process_user_query(question)
